...
def problems_main(questions) :      # hint => function name을 problems_main으로 변경
    ...
    #각 그룹에서 질문 가져옴
    for question in [0,1,2,3]:       # hint => problems_first/second에서 받아야 하는 값이 4개이므로 []안에 인덱스 0, 1, 2, 3를 추가
    ...
    def total_responses(score):
        ...
        if score >= 30:
            score_result = "A"
        elif score >= 20:       # hint => else -> elif로 변경 
            score_result = "B"
        else:
            score_result = "F"
...
if __name__ == "__main__":
    ...
    ]       # hint => 첫번째 질문에 대한 보기 인 '1) var name 2) name = value 3) set name 4) name == value', 추가 
    # 문제 당 정답
    list_corrects = [2, 1, 1, 2]    # hint => 해당 문제들의 정답 추가
